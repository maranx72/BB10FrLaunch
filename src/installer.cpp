...
InstallNet::InstallNet( QString ip, QObject* parent) : QObject(parent),
    ...
    _hadPassword(true), currentBackupZip(nullptr), _zipFile(nullptr), ipaddr(ip), wipecalled(false)
{
#ifdef _MSC_VER
...
/*    QTextStream out(stdout);
    out << "postQuery begin" << endl;
    out << page << endl;
    out << contentType << endl;
            ...
            foreach(QNetworkAddressEntry addr, inter.addressEntries())
            {
                if (addr.ip().protocol() == QAbstractSocket::IPv4Protocol)
                {
                    ...
                    if (addrParts.at(0) > 100) {
                        ...
                        QString maskedip = QString("%1.%2.%3.%4").arg(addrParts.at(0)).arg(addrParts.at(1)).arg(addrParts.at(2)).arg(masked);
...
//    out1 << "discoveryReply begin" << endl;
    ...
    QString dataout = data;
    if (dataout.contains("<BrMetrics>"))
    {
        int startind = dataout.indexOf("<BrMetrics>");
        int len = dataout.indexOf("</BrMetrics>") - dataout.indexOf("<BrMetrics>") + 13; // 13 = </BrMetrics> length 
        dataout = dataout.remove(startind, len);
        dataout = dataout.replace(QString("\n\n"), QString("\n"));
        dataout = dataout.replace(QString("\n\n"), QString("\n"));
//        QTextStream out(stdout);
//        out << startind << "  " << len << endl;
//        out << data << endl;
//       out << dataout << endl;
    }
            ...
            if (xml.isStartElement())
            {
                if (xml.name() == "BbPin") {
                    ...
                    if (!device->setupComplete) {
                        QTextStream out(stdout);
                        out << "Setup not completed on device" << endl;
                        QCoreApplication::exit();
...
void InstallNet::restoreReply()
{
    ...
    {
        QTextStream out(stdout);
//        out << "restoreReply begin" << endl;

        if (reply != nullptr)
        {
            QVariant statustmp = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute);

            if (statustmp.toInt() == QNetworkReply::NoError)
            {
                out << "WIPECALL OK" << endl;
            }
//            QByteArray datatmp = reply->readAll();
//            QString dataouttmp = datatmp;
//            out << dataouttmp << endl;
        }
//        out << "wipecalled -> exit" << endl;
        QCoreApplication::exit();
    }
